...
// Per platform skill, the user sees only the `/` route. All sections render
// as views inside this page, switched by the Zustand store.
//
// Authentication gate: if no session, render LoginView. The server remains
// authoritative — every API route enforces auth via requireAuth().
...
import { VerificationView } from '@/components/cyber/views/VerificationView';
  ...
  if (loading) {
    return (
      <ThemeProvider attribute="class" defaultTheme="dark" enableSystem={false}>
        <div className="min-h-screen flex items-center justify-center bg-background cyber-grid-bg">
          <div className="text-xs font-mono text-muted-foreground animate-pulse">Checking session…</div>
        </div>
      </ThemeProvider>
    );
  }

  // Authentication gate — server remains authoritative.
  if (!user) {
    return (
      <ThemeProvider attribute="class" defaultTheme="dark" enableSystem={false}>
        <LoginView />
...
function Footer() {
  ...
  return (
    <footer className="h-8 border-t bg-background/80 px-4 flex items-center justify-between text-[10px] font-mono text-muted-foreground">
      <div>CYBERSECURITY COMMAND CENTER · SPIRAL 24 · LOCAL-FIRST · EVIDENCE-BASED</div>
      <div>{now}</div>
    </footer>
  );
}
